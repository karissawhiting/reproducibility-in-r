...
[*"Here is my dataset. Make a survival curve."*]{.small}
...
[*"I have a data frame `trial` with: `ttdeath` (time to death in months), `death` (1 = event, 0 = censored), `trt` ('Drug A' / 'Drug B'). Using `{survival}` and `{ggsurvfit}`, write well-commented R code to plot Kaplan-Meier curves by treatment group with a risk table. Then explain each step."*]{.small}
