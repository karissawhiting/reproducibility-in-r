...
# Writing Good Code {background-color="#007CBA" style="text-align: center;"}
...
- Consider [efficiency / readibility]{.emphasized} trade-off
  
- [DNR]{.emphasized} (Do Not Repeat) - if you do it more than 3 times, consider writing a function
...
:::{.column width="50%"}
[**Jupyter (`.ipynb`)**]{.emphasized}

- Primarily Python-first; widely used in data science and ML
- JSON container storing code, text, **and outputs** — **Results are visible on reopen**
...
## Survival outcomes with tbl_survfit(){visibility="hidden"}
...
## {gtsummary} + formulas{visibility="hidden"}
...
## Customize Using Add-on Functions{visibility="hidden"}
...
## Advanced Tips: Update tbl_summary() with modify\_\*(){visibility="hidden"}
...
## Advanced Tips: continuous2 & digits{visibility="hidden"}
...
## Advanced Tips: tbl_continuous(){visibility="hidden"}
...
## Advanced Tips: Custom p-value functions{visibility="hidden"}
...
## Advanced Tip: tbl_merge(){visibility="hidden"}
